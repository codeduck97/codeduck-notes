...
1. Serial（串行GC）	                            —— 标记复制
2. ParNew（并行GC）                            —— 标记复制
3. Parallel Scavenge（并行GC）            —— 标记复制
4. Serial Old（MSC）（串行GC）         —— 标记整理
5. CMS（并发GC）                                 —— 标记清除
6. Parallel Old（并行GC）                     —— 标记整理
...
![image-20200828095122154](images/image-20200828095122154.png)
...
​    **并发标记**：从GC Roots直接关联的对象开始遍历整个对象图，该过程耗时但是可以和用户线程并发执行。
